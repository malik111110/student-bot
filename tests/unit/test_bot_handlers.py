...
class TestBotHandlers:
    ...
    @pytest.fixture
    def mock_update_with_user(self, mock_context):
        """Mock update with user message."""
        user = User(id=12345, first_name="Test", last_name="Student", username="test", is_bot=False)
    ...
    @pytest.mark.asyncio
    async def test_start_command(self, mock_update_with_user, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_help_command(self, mock_update_with_user, mock_context):
        """Test /help command handler."""
        await help_command(mock_update_with_user, mock_context)
        
        mock_context.bot.send_message.assert_called_once()
        call_args = mock_context.bot.send_message.call_args
        assert "commandes" in call_args[1]["text"].lower() or "commands" in call_args[1]["text"].lower()

    @pytest.mark.asyncio
    @patch('core.data_loader.load_json_data')
    @patch('bot.handlers.load_json_data')  # Mock the student check
    async def test_schedule_command(self, mock_student_data, mock_load_data, mock_update_with_user, mock_context):
    ...
    async def test_professors_command(self, mock_load_data, mock_update_with_user, mock_context):
    ...
    @pytest.mark.asyncio
    @patch('bot.handlers.load_json_data')
    async def test_courses_command(self, mock_load_data, mock_update_with_user, mock_context):
    ...
    async def test_news_command(self, mock_scraper, mock_update_with_user, mock_context):
    ...
    @patch('bot.handlers.load_json_data')
    async def test_ai_chat_handler(self, mock_student_data, mock_llm, mock_update_with_user, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_handler_error_handling(self, mock_update_with_user, mock_context):
    ...
    @pytest.mark.asyncio
    @patch('bot.handlers.load_json_data')
    async def test_user_authentication(self, mock_load_data, mock_context):
        ...
        assert "access denied" in call_args.lower() or "not in the students list" in call_args.lower()