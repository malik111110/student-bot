...
def load_seed_module():
    ...
    def dummy_get_client():
        class Dummy:
            postgrest = types.SimpleNamespace(rpc=lambda *a, **k: types.SimpleNamespace(execute=lambda: None))
        return Dummy()
...
def test_validate_programs_against_migration():
    ...
    programs = [
        {'name': 'Test Program', 'extra_field': 'should flag'},  # missing code, has extra
        {'code': 'TEST', 'name': 'Complete', 'description': 'OK', 'department': 'CS'}  # matches schema
    ]
