...
class NewsScraper:
    ...
    def __init__(self):
        ...
        self.field_keywords = {
            "Sécurité informatique": [
                ...
                "malware", "firewall", "penetration testing", "ethical hacking", 
                "data breach", "privacy", "authentication", "cryptography", "infosec"
            ],
            "Intelligence Artificielle": [
                ...
                "robotics", "automation", "algorithm", "data mining", "predictive analytics"
            ],
            "RSD": [  # Réseaux et Systèmes Distribués
                "network", "distributed systems", "cloud computing", "microservices", 
                "kubernetes", "docker", "devops", "infrastructure", "scalability",
                "load balancing", "API", "web services", "system architecture"
            ],
            "Sciences des Données": [
                "data science", "big data", "analytics", "statistics", "python", "R",
                "database", "SQL", "data visualization", "business intelligence", 
                "data mining", "predictive modeling", "machine learning", "AI"
            ],
            "Resin": [  # Réseaux et Systèmes d'Information
                "information systems", "network", "database", "enterprise", "ERP",
                "system administration", "IT management", "infrastructure", 
                "business systems", "data management", "system integration"
            ]
        }
    
    async def scrape_single_url(self, url: str, formats: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.async_firecrawl.scrape(
                url=url,
                formats=formats
            )
            return {
    ...
    async def get_tech_news(self, source: str = "techcrunch", limit: int = 5) -> Dict[str, Any]:
    ...
    async def get_all_tech_news(self, limit_per_source: int = 3) -> Dict[str, Any]:
        ...
        for source_name in ["techcrunch", "arstechnica", "the_verge"]:
            try:
                tech_result = await self.get_tech_news(source_name, limit=limit_per_source)
                results[source_name] = {
    ...
    def filter_content_by_field(self, content: str, field: str) -> str:
        ...
        if relevant_lines:
            ...
            return f"📚 General tech news (filtered for {field}):\n\n" + content[:500] + "..."
        ...
        for source_name, data in all_news.items():
            ...
            if result.get("success") and result.get("content"):
                ...
                if filtered_content and len(filtered_content.strip()) > 50:
                    ...
                    content_parts.append(filtered_content[:300] + "..." if len(filtered_content) > 300 else filtered_content)
...
For the latest news, please try again later or check sources directly.
    ...
    def format_news_for_telegram(self, news_data: Dict[str, Any], max_length: int = 4000) -> str:
    ...
    def _clean_scraped_content(self, content: str) -> str:
        ...
        content = re.sub(r'(Skip to|Jump to|Go to) (main )?content', '', content, flags=re.IGNORECASE)
        content = re.sub(r'(Menu|Navigation|Header|Footer)', '', content, flags=re.IGNORECASE)
        ...
        for line in lines:
            ...
            if line and len(line) > 10:  # Skip very short lines
                ...
                if any(keyword in line.lower() for keyword in ['tech', 'ai', 'software', 'computer', 'security', 'data']):
