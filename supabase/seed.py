...
def resolve_field_code(raw_field: str, field_map: Dict[str, str]) -> str:
    ...
    return field_map.get(raw_field) or field_map.get(key) or field_map.get(raw_field.title()) or raw_field
...
def validate_programs_against_migration(programs, migration_sql_path: str) -> Dict[str, Dict[str, set]]:
...
def import_programs(client, programs_json, *, dry_run=False):
    ...
    programs = programs_json.get('programs') if isinstance(programs_json, dict) and 'programs' in programs_json else programs_json
    results = []
    modules_map = load_modules_map()
    ...
    for p in programs:
        ...
        code = (canon.get('name') or canon.get('title') or 'PROGRAM').upper().replace(' ', '_')[:64]
        payload = [code, canon]
...
def validate_students_json(students) -> dict:
    ...
    if stats['missing_id'] > 0 or stats['missing_name'] > 0 or stats['missing_field'] > 0:
        return {
            'error': (
...
def main(dry_run=False):
    ...
    if ok and programs:
        ...
        migration = os.path.join(os.path.dirname(__file__), 'migrations', '0001_create_tables.sql')
            ...
            for name, validation in results.items():
                if validation['missing_keys'] or validation['extra_keys']:
                    ...
                    if validation['missing_keys']:
                        print("  Missing required columns:", ', '.join(validation['missing_keys']))
                    if validation['extra_keys']:
                        print("  Extra fields that won't be stored:", ', '.join(validation['extra_keys']))
            if has_errors:
                ok = False
